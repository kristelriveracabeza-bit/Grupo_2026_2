...
# DATA SOURCE PARA ZONA HOSPEDADA DE ROUTE 53 (CONDICIONAL)
...
# SECURITY GROUP PARA VPC ENDPOINTS (CREADO PRIMERO)

resource "aws_security_group" "vpc_endpoints" {
  count = var.enable_vpc_endpoints_security_group && (var.enable_sqs_vpc_endpoint || var.enable_sns_vpc_endpoint || var.enable_ecr_vpc_endpoint || var.enable_secretsmanager_vpc_endpoint || var.enable_cloudwatch_vpc_endpoint || var.enable_opensearch_vpc_endpoint || var.enable_elasticache_vpc_endpoint || var.enable_ecs_vpc_endpoint) ? 1 : 0
...
# HELPER LOCAL PARA SG DE VPC ENDPOINTS

locals {
  vpce_sg_ids = var.enable_vpc_endpoints_security_group && (var.enable_sqs_vpc_endpoint || var.enable_sns_vpc_endpoint || var.enable_ecr_vpc_endpoint || var.enable_secretsmanager_vpc_endpoint || var.enable_cloudwatch_vpc_endpoint || var.enable_opensearch_vpc_endpoint || var.enable_elasticache_vpc_endpoint || var.enable_ecs_vpc_endpoint) ? [aws_security_group.vpc_endpoints[0].id] : []
}
...
resource "aws_security_group" "alb" {
  ...
  vpc_id      = aws_vpc.this.id
  description = "Permite tráfico HTTP y HTTPS desde internet"
...
# Security Group para los Microservicios ECS
resource "aws_security_group" "ecs" {
  name        = "${var.project_name}-ecs-sg-${var.environment}"
  vpc_id      = aws_vpc.this.id
  description = "Permite tráfico desde el ALB y servicios internos"
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
    description     = "Tráfico desde el ALB (puerto alternativo)"
  }

  dynamic "ingress" {
    for_each = var.create_redis_security_group ? [1] : []
    content {
      from_port       = 6379
      to_port         = 6379
      protocol        = "tcp"
      security_groups = [aws_security_group.redis[0].id]
      description     = "Redis desde ElastiCache"
    }
  }
...
# Security Group para la Base de Datos (RDS/Aurora)
